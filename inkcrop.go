...
func ditherImage(img image.Image, ditherAlg string, ditherStrength float32) image.Image {
...
func watcherDaemon(input string, matches []string, outdir string, dither *bool, ditherAlg *string, ditherStrength float32, rotate *bool, crop *bool, quality *int) {
	...
	for {
		...
		case event, ok := <-fsWatcher.Events:
			...
			if event.Op&fsnotify.Create == fsnotify.Create {
				...
				processImages(&input, matches, outdir, dither, ditherAlg, ditherStrength, rotate, crop, quality)
			}
...
func processImages(input *string, matches []string, outdir string, dither *bool, ditherAlg *string, ditherStrength float32, rotate *bool, crop *bool, quality *int) {
	...
	for _, filepath := range matches {
		...
		if *dither {
			// Dither the image
			img = ditherImage(img, *ditherAlg, ditherStrength)
		}
		if *rotate {
...
func main() {
	...
	if *ditherAll {
		...
		if *daemon {
			input := *input
			watcherDaemon(input, matches, outdir, dither, ditherAlg, ditherStrength, rotate, crop, quality)
		} else if *link {
			input := *input
			linkTimer := *linkTimer
